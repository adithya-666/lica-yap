...
var DateRangePicker = () => {
  ...
  $("#daterange-picker").on('change', function () {
      ...
      const url = baseUrl('report/test-detail-datatable/'+startDate+'/'+endDate+'/'+typeId);
      Datatable.refreshTableAjax(url);

     
    const printUrl = baseUrl('report/test-detail-print/'+startDate+'/'+endDate+'/'+typeId);
    $('#print-report').attr("href", printUrl);
    const excelUrl = baseUrl('report/test-detail-excel/'+startDate+'/'+endDate+'/'+typeId);
